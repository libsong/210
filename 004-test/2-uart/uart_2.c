...
#define UTRSTAT2 	*((volatile unsigned int *)0xE2900010)
#define UTXH2  		*((volatile unsigned int *)0xE2900020)
#define URXH2 		*((volatile unsigned int *)0xE2900024)
#define UBRDIV2 	*((volatile unsigned int *)0xE2900028)
#define UDIVSLOT2	*((volatile unsigned int *)0xE290002C)
...
{
	while (!(UTRSTAT2 & (1 << 2)));	/* 等待发送缓冲区为空 */
	UTXH2 = byte;					/* 发送一字节数据 */		
}

static unsigned char uart_recv_byte()
{
	while (!(UTRSTAT2 & 1));		/* 等待接收缓冲区有数据可读 */
